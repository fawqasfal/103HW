...
You have 1000 points in the beginning of the game. Given 7 randomly generated letters being mixed with lower and upper cases, 
you need to type them correctly within 7 seconds (7000 msec). 
...
int diff(string attempt, string right) {
	right = replace(right, "[0-9]","0");
	right = replace(right, "[%-&]", "*");
	while (attempt.length() > right.length()) right += " ";
	while (right.length() > attempt.length()) attempt +=  " ";
	int offset = 0;
